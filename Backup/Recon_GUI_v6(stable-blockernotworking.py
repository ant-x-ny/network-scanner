...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.network_addr = tk.StringVar(self, "192.168.18.0/24")
    ...
    def update_tabviews(self):
        ...
        if globalScanned == False:
            ...
            self.selected_blockers = tk.StringVar(self, "1,2,3,4")
            self.selectedDevicesLabel = customtkinter.CTkLabel(self.blocker_tab, textvariable=self.selected_blockers)
            self.selectedDevicesLabel.grid(row=0, column=1, padx=(5, 0), pady=(10, 5), sticky="wn")
            self.blockButton = customtkinter.CTkButton(master=self.blocker_tab, fg_color="#782c2c",text="Block",text_color=("gray10", "#DCE4EE"))
            self.blockButton.grid(row=0, column=3, padx=(0, 0), pady=(10, 5), sticky="wne")
            self.removeblockButton = customtkinter.CTkButton(master=self.blocker_tab, state="disabled",fg_color="#207340",text="Remove Block",text_color=("gray10", "#DCE4EE"))
            ...
            for device in self.devices:
                ...
                self.device_label_checkbox = "device_checkbox_"+str(cache_num)
                ...
                self.device_label_no = customtkinter.CTkLabel(self.blockerDeviceDisplayFrame, text=str(cache_num))
                self.device_label_no.grid(row=cache_num, column=0, columnspan=1,padx=(2,5), pady=(0, 1), sticky="wn")
                
                self.device_label_checkbox = customtkinter.CTkCheckBox(self.blockerDeviceDisplayFrame, text="",onvalue=1, offvalue=0, command=lambda: self.selectDevice(str(self.device_label_checkbox.cget("row")), str(device['ip']), str( self.device_label_checkbox.get())))
                self.device_label_checkbox.grid(row=cache_num, column=1, columnspan=1, padx=(2,5), pady=(0,1), sticky="wn")
                
                self.devIP = tk.StringVar(self, str(device['ip']))
                self.device_label_ip = customtkinter.CTkEntry(self.blockerDeviceDisplayFrame, textvariable=self.devIP, border_width=0, state="readonly", fg_color="transparent")
                self.device_label_ip.grid(row=cache_num, column=2, columnspan=1,padx=(0,0), pady=(0, 1), sticky="wn")
                
                self.devMAC = tk.StringVar(self, str(device['mac']))
                self.device_label_mac = customtkinter.CTkEntry(self.blockerDeviceDisplayFrame, textvariable=self.devMAC, border_width=0, state="readonly", fg_color="transparent")
                self.device_label_mac.grid(row=cache_num, column=3, columnspan=1,padx=(0,0), pady=(0, 1), sticky="wn")
                
                self.devVendor = tk.StringVar(self, str(device['vendor']))
                self.device_label_vendor = customtkinter.CTkEntry(self.blockerDeviceDisplayFrame, textvariable=self.devVendor, border_width=0, state="readonly", fg_color="transparent")
                self.device_label_vendor.grid(row=cache_num, column=4, columnspan=2,padx=(0,0), pady=(0, 1), sticky="wn")
                
                self.devOS = tk.StringVar(self, str(device['os']))
                self.device_label_os = customtkinter.CTkEntry(self.blockerDeviceDisplayFrame, textvariable=self.devOS, border_width=0, state="readonly", fg_color="transparent")
                self.device_label_os.grid(row=cache_num, column=6, columnspan=1,padx=(0,0), pady=(0, 1), sticky="wn")
                
                self.devHostname = tk.StringVar(self, str(device['hostname']))
                self.device_label_hostname = customtkinter.CTkEntry(self.blockerDeviceDisplayFrame, textvariable=self.devHostname, border_width=0, state="readonly", fg_color="transparent")
                self.device_label_hostname.grid(row=cache_num, column=7, columnspan=1,padx=(0,0), pady=(0, 1), sticky="wn")
                cache_num += 1
    ...
    def selectDevice(self, row, ip, checkbox_var):
        print(f"Row: {row}, IP: {ip}, Checked: {checkbox_var.get()}")
    ...
    def arp_spoof(self, target_ip, target_mac, router_ip, our_mac, router_mac): 
        ...
        try:
            ...
            self.restore_arp(target_ip, target_mac, router_ip, router_mac)
